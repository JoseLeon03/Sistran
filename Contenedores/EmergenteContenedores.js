...
function Emergentes (filasTabla,ventanaEmergente){
    ...
    const Estado = fila.querySelector('td:nth-child(9)').textContent;
    ...
    `;
            ...
            const puertoSelect = document.getElementById("puerto");
              ...
              tabla.addEventListener("dblclick", function(event) {
                ...
                if (event.target.tagName === "TD"){
                 ...
                 const bultos = filaSeleccionada.children[10].textContent;
                  const capacidad = filaSeleccionada.children[9].textContent
          ...
          await modificarContenedores({Expediente, Contenedor, Mercancia, Capacidad, Fecha, Recepcion, Puerto, Bultos, Chofer, id});
    ...
    async function modificarContenedores(datos) {
      try {
        console.log(id)
          ...
          const pool = await consultar;
          const sqlQuery = `Update contenedores set Expediente ='${datos.Expediente}', Contenedor = '${datos.Contenedor}', Mercancia = '${datos.Mercancia}', Capacidad = ${datos.Capacidad}, Fecharequerimiento = '${datos.Fecha}', Fecharecepcion = '${datos.Recepcion}', Puertoorigen = ${datos.Puerto}, Bultos = ${datos.Bultos}, Chofer ='${datos.Chofer || ''}' where id = ${id}   `;
          ...
          console.log('Error al agregar el registro:', error);
            ...
            async function recibirContenedor(datos) {
                try {
                    ...
                    const pool = await consultar;
                    const result = await pool.request().query(`SELECT  Format (Fecha , 'yyyy-MM-dd') as Fecha from Viajes where id_viaje = ${datos.id_viaje}`);
                    const viaje = result.recordset[0];

                    const FechaLLegada = viaje.Fecha
            ...
            async function entregarContenedor(datos) {
                try {
                    ...
                    const sqlQuery = `Update contenedores set Fechafin ='${FechaEntrega}', estatus = 5, cod_viaje2 = ${datos.id_viaje} where Contenedor = '${datos.contenedor}'  `;
                    const result2 = await pool.request().query(sqlQuery);
